...
- 홈 커뮤니티 미디어 타일: Mobile `3349:1763396`·`4161:823439`, Tablet `4161:821397`·`4161:823046`, PC `3406:724300`·`4161:820886`
- 홈 카테고리: Mobile `3349:1763363`, PC `3406:733504`
- 프로필 카드: `1021:20324`
- 공용 탭 바: `976:32388`
- 커뮤니티 메인(피드): PC `1440 · 커뮤니티 홈`
- 커뮤니티 작성 폼: `1056:46147`
- 소셜 로그인: PC `3414:750712`, Tablet `3414:751419`, Mobile `3414:751420`
...
| 홈 커뮤니티 타일 | 122×122, 5개 가로 스크롤 |                      122×122, 5개 |               300×300, 4개 |
...
- `CommunityMediaCard`: 홈 `동물 자랑하기`의 정사각 미디어 타일이다. 본문·반응 액션은 커뮤니티 피드 카드에 남기고, 홈에서는 대표 이미지·여러 장 배지만 노출한다. Mobile은 122px 가로 스크롤, Tablet은 122px 5개, PC는 300px 4개를 사용하며 이미지 실패 시 같은 비율의 브랜드 폴백을 유지한다.
...
- `/login`은 Figma 소셜 로그인 3종의 48/64px 헤더, 343/500px 버튼 폭, 12/16px 간격을 따른다. Figma에 남은 이메일·비밀번호 문구와 별도 회원가입 링크는 현행 계약이 아니므로 노출하지 않는다.
- 인증 진입은 Google·Kakao·Naver OAuth만 제공한다. 기존 사용자는 로그인 성공으로, 신규 사용자는 같은 버튼에서 가입 온보딩으로 이어지며 페이지는 이 차이를 미리 선택시키지 않는다.
- 인증 버튼은 소셜 브랜드 고유색을 예외적으로 허용하되 높이 40px, radius 8px, 아이콘·레이블 중앙 묶음, `primary-500` focus-visible ring을 공통으로 유지한다.
- `/login?returnUrl=...`은 `/`로 시작하는 내부 경로만 복귀 대상으로 사용한다. 로그인된 사용자의 재진입은 `replace`, 비로그인 사용자의 브라우저 뒤로가기는 이전 공개 화면으로 복원해 로그인 페이지에 갇히지 않게 한다.
...
| 홈 배너·검색·쇼케이스 | `3349:2252855`, `4042:800891`, `4042:800892`                     | 3개 기준 폭 실측 정렬, 커뮤니티 122/300px 미디어 타일 및 배너 순환 안정화 |
...
| 소셜 로그인           | `3414:750712`, `3414:751419`, `3414:751420`                      | 현행 OAuth 3종·신규 가입 분기·returnUrl·뒤로가기 정렬 완료                |
