...
- Community main uses a clean, text-first feed (user-authorized redesign, 2026-09-13). Its white shell caps at 68rem including gutters. Desktop has a 13rem sticky navigation column, a 3rem gap, and a flexible feed; mobile/tablet use horizontal animal filters above the feed. Search and latest/popular sorting remain next to the content on all screen sizes.
- Main-feed `CommunityFeedCard` uses the opt-in `wide` style: author, multiline text, then photos, followed by reactions. Thin separators replace outlined cards. Photos use a 4:3 frame capped at 26rem with contain fitting on a neutral surface. Other consumers retain the original card style.
- Community main uses the shared `NavigationBar` page title (cafe24, same as My Home, Hall of Fame, Notices) and the shared `SearchBar`. Mobile/tablet animal filters are `FilterChip` (point fill when selected); sorting uses neutral states. Mobile has a floating write action, tablet a header action, and desktop a sidebar action, all with the same login guard.
...
- `ReportAction`: shared report menu, sign-in prompt, visible radio-button reason list, detail input, and result dialog. `ReportPostAction` and `ReportBreederAction` supply their API-specific reasons and submission handlers. Entity cards expose only a `moreAction` slot. The form uses shared `DialogContent`, `DialogHeader`, `DialogTitle`, and `DialogDescription`, including the default 20px mobile / 24px tablet-and-up padding. Primary/outline buttons and the confirmation layer above post detail modals are retained. Reasons are always visible and allow one selection by clicking either the circular radio button or its label.
...
- Breeder reporting is available from the public breeder home and the breeder row on adoption details for adopters; guests are prompted to sign in. It uses `POST /api/v2/adopter/report` with `type: breeder`, `breederId`, the selected reason, and optional description. The `other` reason requires a nonblank description. Community reporting keeps its existing endpoint and duplicate-report response. Pending submissions block duplicate requests and closing; failures preserve the form. Reporting does not immediately remove public content or change profile data, so existing report mutations do not invalidate public read queries.
...
### Activity pages (2026-09-25)

Activity lists and details use the full 1440px shell with the same responsive gutters as their navigation; there is no nested profile-width cap. This is a user-requested exception to the single-column profile cap. `ActivityListLayout` owns the four lists' states and pagination. `ActivityDetailLayout` composes shared navigation, container and async states into a continuous white page, without outlined section cards, shadows or a fixed body height. `ActivityDetailFlow` uses a 320px summary/action column and a flexible reading column at PC, stacking them on mobile/tablet. Application progress and role-specific next steps precede this layout. `ApplicationAnswers` preserves all standard/custom answers, grouped into family, living environment, adoption plans and breeder questions. Received application answers stay visible, followed by status actions and their existing confirmations. Sent review details use the same summary/body layout. Received reviews link to `/activity/received-reviews/[reviewId]`, reusing the existing paginated received-review API and reply actions; no single-review endpoint is assumed. Return links preserve the user's role and sent/received context. Desktop/mobile visual verification of this revision is still pending because the desktop browser provider exposes no windows.
