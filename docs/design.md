...
- hover는 콘텐츠의 위치를 흔들지 않는다. 배경·테두리·그림자 전환을 우선하고, lift가 필요하면 transform으로 레이아웃 크기를 유지한다.
...
- `SearchBar`, `SearchButton`, `SearchIcon`: 검색 submit과 브랜드 톤을 공통화한다.
...
- `ListState`: pending → error → empty → content 우선순위를 공통 처리한다.
...
- 첫 화면의 실제 LCP 이미지만 `priority` 또는 `loading="eager"`를 사용한다.
...
| 영역                  | Figma/공용 기준             | 상태                                                  |
| --------------------- | --------------------------- | ----------------------------------------------------- |
| GNB                   | `3349:1763537`              | 정렬 완료, 회귀 점검 대상                             |
| 홈 배너·검색·쇼케이스 | `3349:1763114`              | 기본 정렬 완료, 이미지 데이터/LCP 점검 필요           |
| 홈 분양 카드          | `3349:1763509`, `MediaCard` | hover·거터 정렬 완료                                  |
...
| 알림                  | 공용 목록·메뉴 패턴         | 기능 연결 완료, 시각/삭제 패턴 정렬 진행 중           |
| 채팅                  | 채팅 공용 폭·Socket.IO      | BaseQi Kafka E2E 및 운영 갭 점검 예정                 |
